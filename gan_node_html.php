...
class DomNode implements IQuery
{
     ...
     * @return DomNode|null
     */
    function &getChild(int|DomNode $child, bool $ignore_text_comments = false): ?DomNode
    {
        if (!is_int($child)) {
     ...
     * @param int|null $offset Position to insert node, negative to count from end, null to append
     * @return DomNode Added node
     */
    function &addChild(string|DomNode $tag, int &$offset = null): string|DomNode
    {
        if (is_array($tag)) {
        ...
        if (is_int($offset) && ($offset < count($this->children)) && ($offset !== -1)) {
            ...
            array_splice($this->children, $offset++, 0, [&$tag]);
        } else {
            $this->children[] =& $tag;
        }
     ...
     * @return DomNode|null
     */
    function &firstChild(bool $ignore_text_comments = false): ?DomNode
    {
     ...
     * @return DomNode|null
     */
    function &lastChild(bool $ignore_text_comments = false): ?DomNode
    {
     ...
     * @return string|DomNode Added node
     * @see addChild();
     */
    function &insertChild(string|DomNode $tag, $offset): string|DomNode
    {
        return $this->addChild($tag, $offset);
    }
    ...
    #php4e
    #php5
    function &addText(string $text, int &$offset = null): string|DomNode
    {
        #php5e
     ...
     * @see addChild();
     */
    function &addComment(string $text, int &$offset = null): string|DomNode
    {
     ...
     * @see addChild();
     */
    function &addConditional(string $condition, bool $hidden = true, int &$offset = null): string|DomNode
    {
     ...
     * @see addChild();
     */
    function &addCDATA(string $text, int &$offset = null): string|DomNode
    {
     ...
     * @see addChild();
     */
    function &addDoctype(string $dtd, int &$offset = null): string|DomNode
    {
     ...
     * @see addChild();
     */
    function &addXML(string $tag = 'xml', string $text = '', array $attributes = [], int &$offset = null)
    {
     ...
     * @see addChild();
     */
    function &addASP(string $tag = '', string $text = '', array $attributes = [], int &$offset = null): string|DomNode
    {
     ...
     */
    function deleteChild(int|DomNode $child, bool $soft_delete = false)
    {
        if (is_object($child)) {
            $child = $this->findChild($child);
        } elseif ($child < 0) {
    ...
    /**
     * Wrap node
     * @param string|DomNode $wrapping_element Wrapping node, string to create new element node
     * @param int $wrap_index Index to insert current node in wrapping node, -1 to append
     * @param int|null $node_index Index to insert wrapping node, null to keep at same position
     * @return string|DomNode Wrapping node
     */
    function wrap($wrapping_element, int $wrap_index = -1, int $node_index = null): string|DomNode
    {
        if ($node_index === null) {
            $node_index = $this->index();
        }

        if (!is_object($wrapping_element)) {
            $wrapping_element = $this->parent->addChild($wrapping_element, $node_index);
        } elseif ($wrapping_element->parent !== $this->parent) {
            $wrapping_element->changeParent($this->parent, $node_index);
        }

        $this->changeParent($wrapping_element, $wrap_index);
     ...
     * @param int|null $node_index Index to insert current node, null to keep at same position
     * @return string|DomNode Wrapping node
     */
    function wrapInner($wrapping_element, int $start = 0, int $end = -1, int $wrap_index = -1, int $node_index = null): string|DomNode
    {
        if ($end < 0) {
            $end += count($this->children);
        }
        ...
        if (!is_object($wrapping_element)) {
            $wrapping_element = $this->addChild($wrapping_element, $node_index);
        } elseif ($wrapping_element->parent !== $this) {
            $wrapping_element->changeParent($this->parent, $node_index);
        }

        $this->moveChildren($wrapping_element, $wrap_index, $start, $end);
        return $wrapping_element;
    }
     ...
     * @param int|string $attr Negative int to count from end
     * @param string $compare "namespace", "name" or "total"
     * @param bool $case_sensitive Compare with case sensitivity
     * @return array|bool array('ns', 'attr', 'ns:attr', index)
     * @access private
     */
    protected function findAttribute(int|string $attr, string $compare = 'total', bool $case_sensitive = false): array|bool
    {
        if (is_int($attr)) {
            if ($attr < 0) {
            ...
            if ($compare === 'total') {
                ...
                if (isset($t[$attr])) {
                    ...
                    if (empty($b[1])) {
                        return [['', $b[0], $attr, $index]];
                    } else {
                        return [[$b[0], $b[1], $attr, $index]];
                    }
                } else {
                    return false;
                }
                ...
                if ($this->attributes_ns === null) {
                    ...
                    foreach ($this->attributes as $a => $v) {
                        $b = explode(':', $a, 2);
                        if (empty($b[1])) {
                            $this->attributes_ns[$b[0]][] = ['', $b[0], $a, $index];
                        } else {
                            $this->attributes_ns[$b[1]][] = [$b[0], $b[1], $a, $index];
                        }
                        ++$index;
                    }
                }
     ...
     * @return bool
     */
    function hasAttribute(int|string $attr, string $compare = 'total', bool $case_sensitive = false): bool
    {
     ...
     * @param string $compare Find node using "namespace", "name" or "total"
     * @param bool $case_sensitive Compare with case sensitivity
     * @return bool|array|string False if not found
     */
    function getAttributeNS(int|string $attr, string $compare = 'name', bool $case_sensitive = false): bool|array|string
    {
     ...
     * @return bool
     */
    function setAttributeNS(int|string $attr, string $namespace, string $compare = 'name', bool $case_sensitive = false): bool
    {
        $f = $this->findAttribute($attr, $compare, $case_sensitive);
        if (is_array($f) && $f) {
            if ($namespace) {
     ...
     */
    function getAttribute(int|string $attr, string $compare = 'total', bool $case_sensitive = false): array|string|null
    {
        $f = $this->findAttribute($attr, $compare, $case_sensitive);
        if (is_array($f) && $f) {
            if (count($f) === 1) {
     ...
     * @param int|string $attr Negative int to count from end
     * @param string|null $val Find node using "namespace", "name" or "total"
     * @param string $compare Find node using "namespace", "name" or "total"
     * @param bool $case_sensitive Compare with case sensitivity
     */
    function setAttribute(int|string $attr, string $val = null, string $compare = 'total', bool $case_sensitive = false)
    {
        if ($val === null) {
     ...
     * @param string $val
     */
    function addAttribute(string $attr, string $val)
    {
     ...
     * @param bool $case_sensitive Compare with case sensitivity
     */
    function deleteAttribute(int|string $attr, string $compare = 'total', bool $case_sensitive = false)
    {
        $f = $this->findAttribute($attr, $compare, $case_sensitive);
        if (is_array($f) && $f) {
            foreach ($f as $a) {
     ...
     * @return bool
     */
    function hasClass($classname): bool
    {
     ...
     * @return array
     */
    function getChildrenByCallback(callable $callback, bool|int $recursive = true, bool $check_self = false): array
    {
     ...
     * @return array
     */
    function getChildrenByMatch($conditions, $recursive = true, $check_self = false, $custom_filters = [])
    {
        $count = $this->childCount();
     ...
     * @access private
     */
    function match($conditions, $match = true, $custom_filters = [])
    {
    ...
    protected function createNodes($content)
    {
        if (is_string($content)) {
            if (strpos($content, ' ') === false) {
                $nodes = [new $this->childClass($content, $this)];
            } else {
...
class EmbeddedNode extends DomNode
{
     ...
     */
    function __construct($parent, $tag_char = '', $tag = '', $text = '', $attributes = [])
    {
        $this->parent = $parent;
...
class XmlNode extends EmbeddedNode
{
     ...
     * @param array $attributes array('attr' => 'val')
     */
    function __construct($parent, $tag = 'xml', $text = '', $attributes = [])
    {
...
class AspEmbeddedNode extends EmbeddedNode
{
     ...
     * @param array $attributes array('attr' => 'val')
     */
    function __construct($parent, $tag = '', $text = '', $attributes = [])
    {
        return parent::__construct($parent, '%', $tag, $text, $attributes);
    }
